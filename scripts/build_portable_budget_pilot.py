...
def portable_leaf(node: dict[str, Any], schema_id: str, question_id: str) -> dict[str, Any]:
    result = {
...
def question_identity(node: dict[str, Any], prefix: str) -> tuple[str, str, str]:
    source_path = normalized_source_path(node["x-source-path"], prefix)
    question_id = f"question:budget:{slug(source_path)}"
...
def replace_input_leaves(
    ...
    *,
    prefix: str,
    questions: dict[str, tuple[str, str, dict[str, Any]]],
) -> None:
    if "x-question-key" in node:
        ...
        if existing is not None:
            existing_comparable = {key: value for key, value in existing[2].items() if key != "$id"}
            if comparable != existing_comparable:
                raise ValueError(f"question constraint drift across profiles: {question_id}")
        else:
...
def bindings_for(schema: dict[str, Any], prefix: str, form_key: str) -> list[dict[str, Any]]:
...
def ui_elements(node: dict[str, Any], base: str = "#/properties") -> list[dict[str, Any]]:
    ...
    for rule in runtime["rules"]:
        ...
        for operand in rule["operands"]:
            ...
            if rule["instance_scope"] == "same_instance":
                ...
                fields.append("@THIS." + dotted(operand_path[len(target_parent_path) :]))
...
def main() -> None:
    ...
    for form_key, profile in PROFILES.items():
        candidate = json.loads((ORACLES / profile["candidate"]).read_text(encoding="utf-8"))
        ...
        replace_input_leaves(schema, prefix=profile["source_prefix"], questions=questions)
        ...
        mappings: dict[str, Any] = {"targets": {"common_grants": {"from": {}, "to": {}}}}
        ...
        generated_forms.append({
            ...
            "source_evidence": [manifest["sources"][key] for key in profile["source_evidence"]],
    ...
    for form in generated_forms:
        ...
        form["supplemental_evidence"][-1] = artifact("evidence/rr-budget-family-profile.json")
