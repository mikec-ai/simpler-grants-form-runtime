...
describe("conditional required rules", () => {
  const conformanceCases = JSON.parse(
    fs.readFileSync(
      path.resolve(
        process.cwd(),
        "../api/tests/fixtures/form_conditional_requiredness_conformance.json",
      ),
      "utf8",
    ),
  ) as {
  ...
  it.each(conformanceCases)(
    ...
    ({ schema, input, expected_required_fields }) => {
      ...
      expect(
        result.warnings.map(({ type, field, message }) => ({
          type,
          field,
          message,
        })),
      ).toEqual(
        expected_required_fields.map((field) => ({
          type: "required",
          field,
          message: `'${field.split(".").at(-1)}' is a required property`,
        })),
      );
    },
  );
