...
import type { UiSchema, WidgetTypes } from "src/types/applyForm/types";
...
export const supportsNativeReadOnly = (widget: WidgetTypes): boolean =>
...
const resolveRef = (
  ...
  if (ref.scope === "root") {
    ...
    const ancestor = ref.ancestor ?? 0;
    scope = itemStack.at(-(ancestor + 1));
  }
  if (!scope) {
